  ...
  private fun distanceMap(input: List<String>, vararg additionalPeople: String): Array<IntArray> {
    ...
    input.map {
      ...
      .forEach { (fromPerson, change, amount, toPerson) ->
        ...
        distanceMap[fromPersonIndex][toPersonIndex] = calculateDistance(distanceMap[fromPersonIndex][toPersonIndex])
        distanceMap[toPersonIndex][fromPersonIndex] = calculateDistance(distanceMap[toPersonIndex][fromPersonIndex])
      }
    return distanceMap
  }
}
