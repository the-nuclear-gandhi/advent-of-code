...
class Year15Day7 : Day<List<String>>() {
  ...
  private fun getWireValue(map: MutableMap<String, String>, wireCode: String): Int {
    ...
    val result = value.toIntOrNull() ?: run {
        ...
        when {
          value.contains("AND") -> (tokens.first().toIntOrNull() ?: getWireValue(map, tokens.first())) and getWireValue(map, tokens.last())
          value.contains("OR") -> (tokens.first().toIntOrNull() ?: getWireValue(map, tokens.first())) or getWireValue(map, tokens.last())
